...
const PengeluaranPage = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/api/pengeluaran')
            .then(response => {
                setPengeluaranData(response.data.data);
                setLoading(false);
            })
            .catch(error => {
    ...
    const totalPengeluaran = () => {
        return filterPengeluaran().reduce((total, item) => total + parseFloat(item.jumlah), 0);
    };
    ...
    return (
        <div className="container mt-4">
            <h4 className="mb-4 text-center">Daftar Pengeluaran</h4>
            ...
            <div className="card shadow-sm">
                <div className="card-header d-flex justify-content-between align-items-center">
                    <h6 className="card-title mb-0">Daftar Pengeluaran</h6>
                ...
                <div className="card-body fs-8">
                    ...
                    <div className="d-flex justify-content-end mt-3 me-2">
                        <h6>Total Pengeluaran: Rp {totalPengeluaran().toFixed(2)}</h6>
                    </div>
                </div>
            </div>
        </div>
